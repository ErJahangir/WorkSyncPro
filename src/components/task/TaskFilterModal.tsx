...
const ToggleChip: React.FC<{
  ...
  return (
    <TouchableOpacity
      ...
      {icon && <RNText style={{fontSize: 13}}>{icon}</RNText>}
      <RNText
        style={{
          fontSize: 13,
...
export const TaskFilterModal: React.FC<TaskFilterModalProps> = ({
  ...
  const SortBtn: React.FC<{by: TaskFilter['sortBy']; label: string}> = ({
    by,
    label,
  }) => (
    <TouchableOpacity
      onPress={() => setLocalFilter(f => ({...f, sortBy: by}))}
      activeOpacity={0.75}
      style={{
        paddingHorizontal: 14,
        paddingVertical: 8,
        ...
        borderWidth: 1.5,
        backgroundColor:
          localFilter.sortBy === by ? theme.colors.primaryLight : 'transparent',
        borderColor:
          localFilter.sortBy === by
    ...
    <Modal
      ...
      <View style={[styles.sheet, {backgroundColor: theme.colors.surface}]}>
        ...
        <View style={styles.sheetHeader}>
          <RNText style={[styles.sheetTitle, {color: theme.colors.text}]}>
            Filter & Sort
          </RNText>
          ...
          <View style={styles.chipRow}>
            {(['low', 'medium', 'high'] as TaskPriority[]).map(p => (
              <ToggleChip
                ...
                selected={localFilter.priority === p}
          ...
          <View style={styles.chipRow}>
            {(['asc', 'desc'] as const).map(order => (
              <ToggleChip
                ...
                onPress={() => setLocalFilter(f => ({...f, sortOrder: order}))}
              />
            ))}
          </View>
